    ...
    <section class="bg-white py-20">
        <div class="max-w-6xl mx-auto px-4">
            <h2 class="text-3xl md:text-4xl font-bold text-center text-gray-900 mb-12">
                De ce să alegi ${product.name || 'acest produs'}?
            </h2>
                ...
                ${faq.map((item, index) => `
                    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <button onclick="toggleFAQ(${index})" 
                                class="w-full text-left px-6 py-4 font-semibold text-gray-900 hover:bg-gray-50 flex justify-between items-center">
                            <span>${item.question || 'Întrebare'}</span>
                            <span class="text-2xl text-blue-500 faq-icon" id="icon-${index}">+</span>
                        </button>
                        <div id="faq-${index}" class="hidden px-6 py-4 text-gray-600 border-t border-gray-200">
                            ${item.answer || 'Răspuns'}
                        </div>
                    </div>
                `).join('')}
