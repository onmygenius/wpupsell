...
// Supports multiple templates: starter (FREE) and professional (PAID)

const { generateProfessionalTemplate } = require('./professional-template');

function generateLandingPageHTML(product, content, store = {}, templateId = 'starter') {
  if (templateId === 'professional') {
    return generateProfessionalTemplate(product, content, store);
  }
...
function generateStarterTemplate(product, content, store = {}) {
  ...
  const storeCurrency = product.currency || store.currency || 'LEI';  // Priority: product > store > LEI
    ...
    <section class="bg-white py-20">
        <div class="max-w-6xl mx-auto px-4">
            <h2 class="text-3xl md:text-4xl font-bold text-center text-gray-900 mb-12">
                ${sectionTitles.whyChoose} ${product.name || 'acest produs'}?
            </h2>
    ...
    <section class="bg-white py-20">
        <div class="max-w-4xl mx-auto px-4 text-center">
            <h2 class="text-3xl md:text-5xl font-bold text-gray-900 mb-6">
                ${sectionTitles.readyToOrder}
            </h2>
            <p class="text-xl text-gray-600 mb-8">
                ${hero.subheadline || 'Comandă acum și bucură-te de calitate premium!'}
            </p>
            <a href="${productUrl}" 
               class="inline-block bg-gray-900 text-white px-12 py-4 rounded-xl text-xl font-bold hover:bg-gray-800 transition-all hover:scale-105 shadow-lg">
                ${hero.cta || 'Cumpără Acum'}
